...
/***/ "./assets/src/js/admin/master-study/migration.js":
/*!*******************************************************!*\
  !*** ./assets/src/js/admin/master-study/migration.js ***!
  \*******************************************************/
...
let masterStudyNode = document.querySelector('.lp-migration-wrapper .content.master_study');
let statusNode;
//course
...
const courseTotal = parseInt(LP_ADDON_IMPORT_EXPORT_GLOBAL_OBJECT?.master_study_course_total) || 0;
const sectionTotal = parseInt(LP_ADDON_IMPORT_EXPORT_GLOBAL_OBJECT?.master_study_section_total) || 0;
const courseItemTotal = parseInt(LP_ADDON_IMPORT_EXPORT_GLOBAL_OBJECT?.master_study_course_item_total) || 0;
const questionTotal = parseInt(LP_ADDON_IMPORT_EXPORT_GLOBAL_OBJECT?.master_study_question_total) || 0;
const courseProcessTotal = parseInt(LP_ADDON_IMPORT_EXPORT_GLOBAL_OBJECT?.master_study_course_process_total) || 0;
...
const migrateMasterStudy = () => {
  if (!masterStudyNode) {
    return;
  }
  autoMigrateBtn = masterStudyNode.querySelector('#lp-auto-migrate-btn.master_study');
  clearDataBtn = masterStudyNode.querySelector('#lp-clear-migrate-btn');
  statusNode = masterStudyNode.querySelector('.status');

  //Course data
  courseNode = masterStudyNode.querySelector('.course');
  ...
  //Section data
  sectionNode = masterStudyNode.querySelector('.section');
  ...
  //Course item data
  courseItemNode = masterStudyNode.querySelector('.course-item');
  ...
  //Question data
  questionNode = masterStudyNode.querySelector('.question');
  ...
  //Course process data
  coursePcNode = masterStudyNode.querySelector('.course-process');
...
const clearData = () => {
  ...
  clearMigratedBtn.addEventListener('click', function (event) {
    ...
    wp.apiFetch({
      path: '/' + restNamespace + '/delete-migrated-data/master-study',
      method: 'DELETE'
    }).then(res => {
      if (res.status && res.status === 'success') {
        ...
        if (res?.data?.master_study_course_total) {
...
const migrate = (paged = 1, item = 'course') => {
  ...
  const number = 10;
  wp.apiFetch({
    path: '/' + restNamespace + '/migrate/master-study',
    method: 'POST',
    data: {
      paged,
      number,
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (migrateMasterStudy);
...
(() => {
/*!***********************************************************!*\
  !*** ./assets/src/js/admin/lp-migration-master-study.tsx ***!
  \***********************************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _master_study_migration__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./master-study/migration */ "./assets/src/js/admin/master-study/migration.js");

document.addEventListener('DOMContentLoaded', event => {
  (0,_master_study_migration__WEBPACK_IMPORTED_MODULE_0__["default"])();
});
})();

/******/ })()
;
//# sourceMappingURL=lp-migration-master-study.js.map