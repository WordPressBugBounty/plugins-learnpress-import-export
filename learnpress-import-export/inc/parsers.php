	...
	function parse( $file ) {
		...
		foreach ( $xml->channel->item as $item ) {
			...
			if ( $post['post_type'] == 'lp_question' ) {
				...
				$post['answers_meta'] = array();

				if ( $wp->answer_meta ) {
					foreach ( $wp->answer_meta as $answer_meta ) {

						$_answer_meta = array(
							'question_id' => (int) $answer_meta->question_answer_id,
							'meta_key'    => (string) $answer_meta->meta_key,
							'meta_value'  => (string) $answer_meta->meta_value,
						);

						$post['answers_meta'][] = $_answer_meta;
					}
				}
			}
			...
			$posts[] = apply_filters( 'learn-press/import/postdata', $post, $wp );
...
class LPR_Export_Import_Parser_Regex {
	...
	function process_post( $post ) {
		...
		if ( $post_type == 'lp_question' ) {
				...
				if ( $answers ) {
					foreach ( $answers as $answer ) {
						$_answer = array(
							...
							'title'       => $this->get_tag( $answer, 'wp:answer_title' ),
							'value'       => $this->get_tag( $answer, 'wp:answer_value' ),
							'order'       => $this->get_tag( $answer, 'wp:answer_order' ),
							'is_true'     => $this->get_tag( $answer, 'wp:answer_is_true' ),
						);
			...
			$postdata['answers_meta'] = array();

			if ( preg_match_all( '|<wp:answer_meta>(.+?)</wp:answer_meta>|is', $post, $answers_meta ) ) {
				$answers_meta = $answers_meta[1];
				if ( $answers_meta ) {
					foreach ( $answers_meta as $answer_meta ) {
						$_answer_meta = array(
							'learnpress_question_answer_id' => $this->get_tag( $answer_meta, 'wp:question_answer_id' ),
							'meta_key'   => $this->get_tag( $answer_meta, 'wp:meta_key' ),
							'meta_value' => $this->get_tag( $answer_meta, 'wp:meta_value' ),
						);

						$postdata['answers_meta'][] = $_answer_meta;
					}
				}
			}
		}
